...
class UserManager(models.Manager):
    def basic_validator(self, post_data):
        ...
        if not post_data['birthday']:
            errors['no_birthday']="No birthday provided"
        else:
            today = datetime.now()
            thirteen = datetime(today.year - 13, today.month, today.day)
            if datetime.strptime(post_data['birthday'], "%Y-%m-%d") > thirteen:
...
class User(models.Model):
    ...
    birthday = models.DateTimeField(default = "2000-04-01")
